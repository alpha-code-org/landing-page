...
export function useMDXComponents(components: MDXComponents): MDXComponents {
  return {
    ...
    code: ({ children }) => (
      <code className="rounded-md bg-stone-200/60 px-1.5 py-0.5 font-mono text-sm text-stone-800 dark:bg-neutral-700 dark:text-stone-200">
        {children}
      </code>
    ),
    pre: ({ children }) => (
      <pre className="mb-6 overflow-x-auto rounded-xl border border-stone-200 bg-stone-100 p-4 text-sm dark:border-neutral-600 dark:bg-neutral-700">
        {children}
      </pre>
    ),
    blockquote: ({ children }) => (
      <blockquote className="my-8 rounded-md border-l-4 border-stone-300 bg-stone-100 px-6 py-4 text-stone-700 italic dark:border-neutral-600 dark:bg-neutral-800/60 dark:text-stone-300 [&>p]:mb-0 [&>p]:text-inherit">
        {children}
      </blockquote>
    ),
    ul: ({ children }) => (
